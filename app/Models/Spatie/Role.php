...
use App\Models\MessageNotification\BulkNotificationRole;
...
class Role extends BaseModel implements RoleContract
{
     ...
     * @return \Spatie\Permission\Contracts\Role|\App\Models\Spatie\Role
     *
    ...
    public function scopeWebActiveRoles($query, $franchiseID = '', $forModifications = false): Collection
    {
        $query->where(['guard_name' => 'web', 'is_active' => 1]);
        $userFranchiseID = auth('web')->user()->franchise_id;

        if(!empty($franchiseID)) {
            $query->where('franchise_id', $franchiseID);
        }

        if ($userFranchiseID != FranchiseType::MAIN->value) { // not a main franchise, will return only own roles
            $query->where('franchise_id', $userFranchiseID);
        }

        if ($forModifications) {
            $query->whereNotIn('id', [DefaultRoleType::SUPER_ADMIN->value]);
        }
    ...
    public function bulkNotificationRoles(): HasMany
    {
        return $this->hasMany(BulkNotificationRole::class, 'bulk_notification_id', 'id');
    }
}
